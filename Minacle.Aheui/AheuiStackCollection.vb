Public Class AheuiStackCollection
  Implements IAheuiStorageCollection(Of Stack(Of Integer))

  Public Sub Add(token As FinalNames) Implements ICollection(Of FinalNames).Add
    collection.Add(token, New Stack(Of Integer))
  End Sub

  Public Sub Add(ParamArray tokens As FinalNames()) Implements IAheuiStorageCollection(Of Stack(Of Integer)).Add
  ...
  Default Public ReadOnly Property Item(token As FinalNames) As Stack(Of Integer) Implements IAheuiStorageCollection(Of Stack(Of Integer)).Item
    Get
  ...
  Public Function Remove(ParamArray tokens As FinalNames()) As Boolean Implements IAheuiStorageCollection(Of Stack(Of Integer)).Remove
    If IsReadOnly Then Throw New NotSupportedException
    Try
      For Each token In tokens
        collection.Remove(token)
